...
if(isset($_POST['action'])){
	...
	switch ($objeto){
		...
		break;
		case 'Articulos':
		$titles='<tr><th>C-ARTI</th><th class="hidden">C-BARRA</th><th>COMERCIAL</th><th>GENERICO</th><th class="hidden">IVA</th><th class="hidden">UND</th></tr>';
		break;
		case 'ODC':
	...
	if($objeto=='Almacenes'){
		...
		$art = new articulo();
		$dat_art = $art->list_art_comer();
		if (!empty($dat_art)){
			foreach ($dat_art as $key3 => $value3){
				$row++;
				$row_clas = ($row%2==0) ? 'odd' : 'even' ;
				$table.='<tr class="gradeA '.$row_clas.'"><td class="'.$objeto.'_id">'.$value3['cart_comercial'].'</td><td class="'.$objeto.'_code hidden">'.$value3['cbarra'].'</td><td class="'.$objeto.'_name">'.$value3['art_comercial'].'</td><td class="'.$objeto.'_gen">'.$value3['generico'].'</td><td class="'.$objeto.'_iva hidden">'.$value3['iva'].'</td><td class="'.$objeto.'_und hidden">'.$value3['cunidad'].'</td></tr>';
			}
		}
	}
	else if($objeto=='ODC'){
